	   ...
	   success:function(data){
		   if(data.successCode=="yes"){
			   if(group.length>18){
					  ...
					  $('<span>',{class:"storage",id:data.GroupCode,text:group}).appendTo(li);
			   }
			   else{
				   let ul = $('<ul>', {class: 'groupFolder'})
				   $(ul).insertBefore($('.add').closest('.groupFolder'));
...
//		   $('<input>',{id:"indexPopupText",type:"text"}).appendTo($('<ul>').appendTo('.indexPopup'))
      ...
      $(document).on('mousedown','#expelGroupMember',function() {
            ...
            $.ajax ({
               ...
               success: function(leaderCheck, object) {
                if(leaderCheck=="true" && object=="me") {
                   $('#dynamicMessage').empty();
                   $('<li>', {text: "자기 자신을 추방할 수 없습니다."}).prependTo(ul);
                   $('.indexMessagePopup').css("display", "block");
                   $(document).on("mouseover", "#indexMessagePopupOkayBtn", function() {
                      $('.indexMessagePopup').hide();
                   })
                   isMenuBoxOn = false;
                }
                else if(leaderCheck=="true" && object=="other") {
                   $('#dynamicMessage').empty();
                   $('<li>', {text:"추방이 완료되었습니다."}).prependTo(ul);
                ...
                else if(leaderCheck=="false" && object==null) {
                   $('#dynamicMessage').empty();
                   $('<li>', {text: "일반 회원은 추방을 수행할 수 없습니다."}).prependTo(ul);
                   $('.indexMessagePopup').css("display", "block");
                   $(document).on("mouseover", "#indexMessagePopupOkayBtn", function() {
                      $('.indexMessagePopup').hide();
                   })
