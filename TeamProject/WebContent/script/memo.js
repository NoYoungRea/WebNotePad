	...
	$(document).on('keydown','#title',function(key) {
			...
			success:function(data){
				...
				for(let i=0;i<data.myGroupList.length;i++){
					let box=$('<div>',{class:"editorBox"}).appendTo($('.right'));
	 	    		...
	 	    		$('<div>',{id:"regDate",contentEditable:false,text:data.myGroupList[i].regdate}).appendTo(h);
	 ...
	 $(document).on("click", ".dateFolder", function() {
	         ...
	         success: function(data) {
	            ...
	            for(let i = 0;i<data.contentName.length;i++){	
	            	let box=$('<div>',{class:"editorBox"}).appendTo($('.right'));
	 	    		...
	 	    		$('<div>',{id:"regDate",contentEditable:false,text:data.contentName[i].regdate}).appendTo(h);
	   ...
	   $(document).on("click",'#editBtn',function(){
		   ...
		   if($(this).siblings('#editor').children('#title').attr('contenteditable')=='false'){
			   $(this).siblings('#editor').children('#title').attr('contenteditable',true)
			   console.log($(this).siblings('#editor').children('#title').attr('contenteditable'));
			  $(this).siblings('#editor').find('#shdudfo').attr('contenteditable',true)
			  $(this).text("수정완료");
		   }
		   else{
			   $(this).siblings('#editor').children('#title').attr('contenteditable',false)
